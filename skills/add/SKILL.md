...
5. Run `node <PLUGIN_ROOT>/scripts/store.js --file <that path>`. On success, delete the tmp file.
...
| Code | What to do |
|---|---|
| `OLLAMA_DOWN` | Tell the user to start Ollama (`ollama serve` or the app). The transit JSON stays in `~/.lookout/tmp/` — re-run the same `store.js --file` after Ollama is up, no need to re-summarize. |
